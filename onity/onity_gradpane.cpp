...
//dialog
*/
class onity_setval_dialog : public xui_dialog
{
	xui_declare_rtti
...
xui_implement_rtti(onity_setval_dialog, xui_dialog);
xui_implement_rtti(onity_gradpane,		xui_drawer);

/*
//constructor
*/
xui_create_explain(onity_gradpane)( s32 flowstyle )
...
, m_ratio(1.0)
{
	...
	xui_method_ptrcall(m_setval, xm_click) += new xui_method_member<xui_method_args, onity_gradpane>(this, &onity_gradpane::on_menuclick);
	xui_method_ptrcall(m_delete, xm_click) += new xui_method_member<xui_method_args, onity_gradpane>(this, &onity_gradpane::on_menuclick);
	xui_method_ptrcall(m_delall, xm_click) += new xui_method_member<xui_method_args, onity_gradpane>(this, &onity_gradpane::on_menuclick);
}

/*
//destructor
*/
xui_delete_explain(onity_gradpane)( void )
{
...
//method
*/
xui_method_explain(onity_gradpane, get_lines,			const std::vector<s32>&	)( void ) const
{
	return m_linearray;
}
xui_method_explain(onity_gradpane, del_lines,			void					)( void )
{
	m_linearray.clear();
	m_downindex = -1;
}
xui_method_explain(onity_gradpane, get_trans,			s32						)( void ) const
{
	return m_trans;
}
xui_method_explain(onity_gradpane, set_trans,			void					)( s32 trans )
{
	m_trans = trans;
}
xui_method_explain(onity_gradpane, get_ratio,			f64						)( void ) const
{
	return m_ratio;
}
xui_method_explain(onity_gradpane, set_ratio,			void					)( f64 ratio )
{
	m_ratio = ratio;
}
...
//override
*/
xui_method_explain(onity_gradpane, on_mousedown,		void					)( xui_method_mouse& args )
{
...
xui_method_explain(onity_gradpane, on_mousemove,		void					)( xui_method_mouse& args )
{
	...
	else
	{
		for (u32 i = 0; i < m_linearray.size(); ++i)
		{
				...
				m_linearray[i] <= line+1)
			{
				switch (m_flowstyle)
				{
				case FLOWSTYLE_H: m_cursor = CURSOR_WE; break;
				case FLOWSTYLE_V: m_cursor = CURSOR_NS; break;
				}
...
xui_method_explain(onity_gradpane, on_mouserise,		void					)( xui_method_mouse& args )
{
...
xui_method_explain(onity_gradpane, on_mousedoubleclick, void					)( xui_method_mouse& args )
{
...
xui_method_explain(onity_gradpane, on_renderself,		void					)( xui_method_args&  args )
{
	xui_drawer::on_renderself(args);

	s32 grap	= (s32)(ceil(20.0/m_ratio/5.0) * 5.0);
	s32 gradidx = -m_trans / grap - 1;
	s32 gradnum = 0;
	switch (m_flowstyle)
	{
	case FLOWSTYLE_H: gradnum = (s32)(ceil((f64)get_renderw() / (grap * m_ratio))); break;
	case FLOWSTYLE_V: gradnum = (s32)(ceil((f64)get_renderh() / (grap * m_ratio))); break;
	}
	...
	for (s32 i = 0; i < gradnum+1; ++i)
	{
		...
		case FLOWSTYLE_H: 
			...
			break;
		case FLOWSTYLE_V:
			p1 = pt + xui_vector<s32>(length,		position);
			p2 = pt + xui_vector<s32>(get_renderw(),position);
			break;
		}
		...
		case FLOWSTYLE_V:
			a  = xui_pi/2.0f;
			tp = pt + xui_vector<s32>(20, position+2);
			break;
		}
	...
	for (u32 i = 0; i < m_linearray.size(); ++i)
	{
		...
		s32 position = (s32)((line+m_trans) * m_ratio);
		xui_vector<s32> p1;
		xui_vector<s32> p2;
		switch (m_flowstyle)
		{
		case FLOWSTYLE_H: 
			...
			break;
		case FLOWSTYLE_V:
			p1 = pt + xui_vector<s32>(24,			position);
			p2 = pt + xui_vector<s32>(get_renderw(),position);
			break;
		}
...
//event
*/
xui_method_explain(onity_gradpane, on_menuclick,		void					)( xui_component* sender, xui_method_args& args )
{
	if (sender == m_setval)
	{
		...
		{
			onity_setval_dialog* dialog = new onity_setval_dialog;
			xui_desktop::get_ins()->add_child(dialog);
			dialog->set_value(index, m_linearray[index]);
			dialog->xm_accept += new xui_method_member<xui_method_args, onity_gradpane>(this, &onity_gradpane::on_acceptsetval);
		}
	}
	else
	if (sender == m_delete)
...
xui_method_explain(onity_gradpane, on_acceptsetval,		void					)( xui_component* sender, xui_method_args& args )
{
	onity_setval_dialog* dialog = xui_dynamic_cast(onity_setval_dialog, sender->get_window());
