...
xui_implement_rtti(onity_recent, xui_window);

/*
//constructor
*/
xui_create_explain(onity_recent)( void )
: xui_window(xui_vector<s32>(480, 640))
{
	m_modal		= true;
	...
	xui_method_ptrcall(m_recent, xm_mousedoubleclick) += new xui_method_member<xui_method_mouse, onity_recent>(this, &onity_recent::on_recentdoubleclick);
...
//method
*/
xui_method_explain(onity_recent, get_selectpath,		std::wstring)( void )
{
	std::wstring path;
...
//callback
*/
xui_method_explain(onity_recent, on_accept,				void		)( xui_component* sender, xui_method_args&  args )
{
	save_config();
...
xui_method_explain(onity_recent, save_config,			void		)( void )
{
	...
	if (file.Open("recent.txt", NPFile::OM_WRITE))
	{
		...
		for (u32 i = 0; i < m_recent->get_upmostnodecount(); ++i)
		{
			...
			xui_treedata* data = node->get_linkdata();
			file.WriteLine(xui_global::unicode_to_ascii(data->get_text(0)));
		}
