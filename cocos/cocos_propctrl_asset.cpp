...
xui_implement_rtti(cocos_propctrl_asset,	xui_propctrl_object);

/*
//constructor
*/
xui_create_explain(cocos_propctrl_asset)( xui_propdata* propdata )
: xui_propctrl_object(propdata)
{
	xui_control* editctrl = m_propedit->get_editctrl();
	editctrl->xm_mouseenter += new xui_method_member<xui_method_mouse, cocos_propctrl_asset>(this, &cocos_propctrl_asset::on_editctrlmouseenter);
	editctrl->xm_mouseleave += new xui_method_member<xui_method_mouse, cocos_propctrl_asset>(this, &cocos_propctrl_asset::on_editctrlmouseleave);
}

/*
//static
*/
xui_method_explain(cocos_propctrl_asset,	create, xui_propctrl*)( xui_propdata* propdata )
{
	return new cocos_propctrl_asset(propdata);
}

/*
//event
*/
xui_method_explain(cocos_propctrl_asset,	on_editctrlmouseenter,	void				)( xui_component* sender, xui_method_mouse& args )
{
	bool same = true;
	xui_propdata_object* dataobject = dynamic_cast<xui_propdata_object*>(m_propdata);
	if (dataobject->has_droptype("ParticleSystem")	== false &&
		dataobject->has_droptype("SpriteFrame")		== false &&
		dataobject->has_droptype("Texture2D")		== false)
		return;
