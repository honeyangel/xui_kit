...
xui_implement_rtti(cocos_savectrl, xui_kindctrl);

/*
//constructor
*/
xui_create_explain(cocos_savectrl)( xui_propkind* propkind )
: xui_kindctrl(propkind)
{
	m_save = new xui_button(xui_vector<s32>(80, 20));
	xui_method_ptrcall(m_save,	xm_buttonclick	) += new xui_method_member<xui_method_args, cocos_savectrl>(this, &cocos_savectrl::on_saveclick);
	...
	xui_method_ptrcall(m_load,	xm_buttonclick	) += new xui_method_member<xui_method_args, cocos_savectrl>(this, &cocos_savectrl::on_loadclick);
...
//static
*/
xui_method_explain(cocos_savectrl, create,				xui_kindctrl*	)( xui_propkind* propkind )
{
	return new cocos_savectrl(propkind);
}

/*
//callback
*/
xui_method_explain(cocos_savectrl, on_updateself,		void			)( xui_method_update& args )
{
	xui_kindctrl::on_updateself(args);
	bool modify = false;
	cocos_savekind* savekind = dynamic_cast<cocos_savekind*>(m_propkind);
	if (savekind)
	{
		cocos_propfile* propfile = savekind->get_propfile();
...
xui_method_explain(cocos_savectrl, on_perform,			void			)( xui_method_args&   args )
{
...
//event
*/
xui_method_explain(cocos_savectrl, on_saveclick,		void			)( xui_component* sender, xui_method_args& args )
{
	for (u32 i = 0; i < m_propkindvec.size(); ++i)
	{
		cocos_savekind* savekind = dynamic_cast<cocos_savekind*>(m_propkindvec[i]);
		cocos_propfile* propfile = savekind->get_propfile();
...
xui_method_explain(cocos_savectrl, on_loadclick,		void			)( xui_component* sender, xui_method_args& args )
{
	for (u32 i = 0; i < m_propkindvec.size(); ++i)
	{
		cocos_savekind* savekind = dynamic_cast<cocos_savekind*>(m_propkindvec[i]);
...
//override
*/
xui_method_explain(cocos_savectrl, get_elsectrlsize,	s32				)( void )
{
	return KIND_HEIGHT;
}
xui_method_explain(cocos_savectrl, get_prevctrlsize,	s32				)( void )
{
	return KIND_HEIGHT;
}