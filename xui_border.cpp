...
//constructor
*/
xui_create_explain(xui_border)( const xui_vector<s32>& size, xui_component* parent, u08 flag )
...
xui_method_explain(xui_border, on_topdraw,		void)( xui_method_args&  args )
{
	...
	if (m_parent)
	{
		...
		case RESIZEFLAG_L:
		case RESIZEFLAG_R:
			rt.oft_x(pt.x - m_down.x);
			break;
		case RESIZEFLAG_T:
		case RESIZEFLAG_B:
			rt.oft_y(pt.y - m_down.y);
			break;
		}

		xui_convas::get_ins()->fill_rectangle(rt, xui_colour(0.5f, 0.0f, 0.0f, 0.0f));
	}
}