...
//constructor
*/
xui_create_explain(cocos_backup)( void )
: xui_window(xui_vector<s32>(480, 640), true)
{
	...
	xui_method_ptrcall(m_selectall, xm_toggleclick		) += new xui_method_member<xui_method_args, cocos_backup>(this, &cocos_backup::on_toggleclick);
	...
	xui_method_ptrcall(m_accept,	xm_buttonclick		) += new xui_method_member<xui_method_args, cocos_backup>(this, &cocos_backup::on_buttonclick);
...
//callback
*/
xui_method_explain(cocos_backup, on_updateself,		void)( xui_method_update& args )
{
...
xui_method_explain(cocos_backup, on_accept,			void)( xui_component* sender, xui_method_args& args )
{
	restore();
...
//event
*/
xui_method_explain(cocos_backup, on_buttonclick,	void)( xui_component* sender, xui_method_args& args )
{
	if (sender == m_accept)
	{
...
xui_method_explain(cocos_backup, on_toggleclick,	void)( xui_component* sender, xui_method_args& args )
{
	for (u32 i = 0; i < m_backup->get_upmostnodecount(); ++i)
	{
...
//method
*/
xui_method_explain(cocos_backup, restore,			void)( void )
{
	for (u32 i = 0; i < m_backup->get_upmostnodecount(); ++i)
	{
...
xui_method_explain(cocos_backup, load,				void)( void )
{
