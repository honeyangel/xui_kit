...
xui_method_explain(onity_restore, load_unsavedfiles,	void)( void )
{
	...
	for (u32 i = 0; i < mainform->get_unsavedfilenum(); ++i)
	{
		//m_restore->add_upmostnode(i, new xui_treedata(mainform->get_unsavedfile(i), true));
	}
}
...
{
	for (u32 i = 0; i < m_restore->get_upmostnodecount(); ++i)
	{
